...
class Scraper:
    def __init__(self):
        ...
        self.cookies_url = (
            "https://www.ss.com/lv/real-estate/flats/riga/all/fDgQeF4S.html"
        )
    ...
    def scrape_page(self):
        ...
        for data in marker_data:
            ...
            if validCost(elements[6]):
                ...
                cost = elements[6].split(" ")[1]
                if self.validNeighbourhood(coordinates):
                    print(" | ".join([neighbourhood, address, rooms, floor[0], cost]))
    ...
    def validNeighbourhood(self, coordinates: tuple) -> bool:
        ...
        for polygon in self.bad_coordinates:
            # print("CHECKING: ", coordinates, polygon)
            if polygon.contains(point):
                return False
        return True

def validCost(cost: str) -> bool:
    try:
        price = int(cost.split(" ")[1].replace(",", ""))
        if price <= 100000 and price >= 30000:
            return True
    except ValueError:
        return False
    return False
